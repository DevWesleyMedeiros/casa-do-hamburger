 ...
 * (RF-52, RN-AUTH-08).
 *
 * lê variáveis de ambiente, valida se estão presentes, nunca expõe a
 * service account fora deste módulo.
 *
 * ⚠️ RNF-24 (revisado v1.8.0): sob Firebase Authentication não existe
 * CLIENT_SECRET de OAuth manuseado pelo projeto — a credencial sensível
 * aqui é a Service Account do Firebase Admin. Ela nunca deve ir para o
 * frontend nem ser commitada — só existe como variável de ambiente do
 * backend (Railway).
 */
...
// Wrapper fino — quem consome (googleAuth.service.ts) não precisa saber
// como o Firebase Admin foi inicializado, só chama verifyIdToken.
export const verifyFirebaseIdToken = async (idToken: string) => {
  ...
  // (ex.: usuário deslogado de todos os dispositivos) — mais uma
  // chamada de rede, mas fecha uma janela de reuso de token roubado
