...
export const useCartItemMutations = (id: string, quantity: number) => {
  ...
  const withOptimisticUpdate = (
    ...
    onMutate: () => {
      const snapshot = queryClient.getQueryData<CartItemType[]>(queryKeys.cartItems);
      queryClient.setQueryData<CartItemType[]>(queryKeys.cartItems, (old = []) => updater(old));
      return { snapshot };
    },
    onError: (_err: unknown, _vars: unknown, context?: { snapshot?: CartItemType[] }) => {
  ...
  const increment = useMutation({
    ...
    ...withOptimisticUpdate('increment', (old) =>
      old.map((item) => (item.id === id ? { ...item, quantity: quantity + 1 } : item)),
    ),
  });
  ...
  const deleteItem = useMutation({
    ...
    ...withOptimisticUpdate('delete', (old) => old.filter((item) => item.id !== id)),
