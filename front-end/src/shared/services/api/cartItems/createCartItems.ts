...
export const NewCartItem = {
  newCartItem: async (payload: CreateCartItemPayload): Promise<CartItemType | ApiError> => {
    try {
      const { data } = await api.post('/auth/create-cart-item', payload);
      return data;
    } catch (error) {
      if (axios.isAxiosError(error)) {
        const message = error.response?.data?.message ?? 'Erro desconhecido ao fazer login';
