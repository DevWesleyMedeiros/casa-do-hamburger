...
export const resetPassword = {
  reset: async (payload: { token: string; newPassword: string }): Promise<AuthMessageResponse> => {
    try {
      const { data } = await api.post<AuthMessageResponse>('/auth/reset-password', payload);
      return data;
    } catch (error) {
      if (axios.isAxiosError(error)) {
        const message = error.response?.data?.message ?? 'Erro desconhecido ao redefinir a senha';
