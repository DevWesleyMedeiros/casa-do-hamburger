...
export const Products = ({ id, name, description, price, images }: ProductsInterface) => {
    ...
    async (productId: string) => {
      ...
      try {
        ...
        if (response instanceof ApiError) {
          toast.error(resolveApiErrorMessage(response, { 404: 'Produto não encontrado' }));
          return;
        }
        ...
        <div className="flex w-full flex-col justify-between">
          <div>
            <div className="flex justify-between">
              <p className="my-0.5 text-sm font-bold uppercase md:text-lg">{name}</p>
              {user?.admin && (
            ...
            <p className="md:text-md flex-1 text-xs text-[#848484]">{description}</p>
          </div>
