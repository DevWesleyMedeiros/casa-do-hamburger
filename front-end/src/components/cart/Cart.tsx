  ...
  const createOrderMutation = useMutation({
    ...
    onError: (err: unknown) => {
      if (err instanceof ApiError) {
        toast.error(
          resolveApiErrorMessage(err, "Não foi possível finalizar o pedido"),
        );
      ...
      >
        ...
        <div className="mx-3 flex flex-1 flex-col gap-2.5">
          {isLoading ? (
            ...
            <p className="text-brand-dark text-center text-sm">
        ...
        <Button
          ...
          disabled={
            isLoading || cartItems.length === 0 || createOrderMutation.isPending
          }
