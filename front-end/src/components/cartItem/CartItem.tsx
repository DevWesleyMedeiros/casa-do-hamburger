...
export const CartItem = ({ id, name, price, images, quantity }: CartItemProps) => {
  const subtotal = price * quantity;
  const { increment, decrement, deleteItem } = useCartItemMutations(id, quantity);
  ...
  return (
    <div className="my-component-card flex items-center justify-between">
      <div className="img">
        <img src={getProductImageUrl(images, 'thumbnail')} alt={name || 'imagem do item'} />
      </div>
      ...
      <div className="texto mx-2 flex flex-1 flex-col gap-0.5 py-1.5">
        <p className="title text-brand-dark text-sm font-bold uppercase">{name}</p>
        <p className="price text-sm text-[#848484]">{brazilinaCurrencyFormat(subtotal)}</p>
      ...
      <div className="inset-0 cursor-pointer">
        <Trash2 size={ICON_CONFIG.mxSize} onClick={() => deleteItem.mutate(id)} />
      </div>
    </div>
  );
};
